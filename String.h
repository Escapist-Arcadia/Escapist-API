...
private:
	...
	using Self = typename BasicString<Char>;
	...
	static inline constexpr SizeType Capacity(SizeType initialSize) {
		if (initialSize) { // However, if the input is zero, we don't need to allocate them.
			//++initialSize;
			if (initialSize + 1 > MinimumCapacity) {
		...
		if (growthSize) {
			if (ref && data && size) {
				...
				if (*ref && (**ref).IsShared()) {
					...
					Construct(size + growthSize, nullptr); // Then create a new object like constructor.
...
public:
	...
	BasicString(const Char* initialData) {
		if (initialData) {
			Construct(CharTrait::GetLength(initialData), nullptr);
	...
	BasicString(const Char* initialData, SizeType dataSize, SizeType dataOffset) {
		if (initialData && dataSize) {
			Construct(dataSize + dataSize, nullptr);
	...
	Self& Append(const Char& ch) {
		if (ch) { // Do I need to check whether the char is 0?
			CharTrait::Assign(GrowthAppend(1), ch);
		}
		return *this;
	}
	Self& Append(const Char& ch, const SizeType count) {
		if (ch && count) {
			CharTrait::Fill(GrowthAppend(count), ch, count);
		}
		return *this;
	}
	Self& Append(const Char* initialData) {
		if (initialData) {
			...
			CharTrait::Copy(GrowthAppend(length), initialData, length);
		}
		return *this;
	}
	Self& Append(const Char* initialData, const SizeType dataSize) {
		if (initialData && dataSize) {
			CharTrait::Copy(GrowthAppend(dataSize), initialData, dataSize);
		}
		return *this;
	}
	Self& Append(const Self& other) {
		if (!other.IsEmptyOrNull()) {
			CharTrait::Copy(GrowthAppend(other.size), other.data, other.size);
		}
		return *this;
	}
	...
	Self& Assign(const Char* newData) {
		return Assign(newData, CharTrait::GetLength(newData));
	}
	Self& Assign(const Char* newData, SizeType dataSize) {
		if (newData && dataSize) {
			if (ref && data && size) {
				if (*ref && (**ref).IsShared()) {
					(**ref).DecrementRef();
					Char* const oldData = data;
					Construct(dataSize, nullptr);
					CharTrait::Copy(data, newData, dataSize);
				}
				else {
					size = dataSize;
					if (size > capacity) {
				...
				new(this)Self(newData, dataSize);
			}
		}
		else {
			Self::~BasicString();
	...
	Self& Assign(const Self& other, SizeType dataSize) {
		if (dataSize && other.ref && other.data && other.size) {
			if (ref && data && size) {
				return Assign(other.data, dataSize);
			}
			else {
				new(this)Self(other, dataSize);
			}
		}
		return *this;
	}
	...
	Self& Replace(const Char* oldStr, const Char* newStr) {
		/*if (oldStr && newStr && !IsEmptyOrNull()) {
			...
			if (*ref && (**ref).IsShared()) {
				Char* const oldData = data;
				(**ref).DecrementRef();
				Construct(size + (newStrLen - oldStrLen) * count);
				while (pos = CharTrait::IndexOf(pos, oldStr)) {
	...
	Self GetRight(const Char* right) {
		if (right) {
			SizeType index = LastIndexOf(right);
	...
	Self Trim()const {
		if (!IsEmptyOrNull()) {
			SizeType index = FirstNotOf(Char(' '));
			SizeType lastIndex = LastNotOf(Char(' '));
...
using String = typename BasicString<Char>;